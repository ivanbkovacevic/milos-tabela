...
function ContextProvider(props: React.PropsWithChildren<{}>) {
  ...
  const addNewProject = async (data: Project) => {
    try {
      ...
      if ( typeof(data.productImg) === 'object' && data.productImg !== null ) {
        ...
        axios.post("/.netlify/functions/server/newProject", addedImgPath);
      }
    } catch (error) {
  ...
  const editProject = async (data: Project) => {
    try {
      ...
      await axios.post(`/.netlify/functions/server/editProject`, addedImgPath);
      }
    } catch (error) {
  ...
  const removeProject = async (data: Project | null) => {
    try {
      await axios.post(`/.netlify/functions/server/deleteProject`, data);
    } catch (error) {
      console.error("Error editing item", error);
    }
    setState({
